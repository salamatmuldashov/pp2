...
class Game:
    ...
    def play(self):
        ...
        self.display_score()
        self.display_level()
        self.snake.add = random.randint(1,3)
        pygame.display.flip()
        time.sleep(self.sleep)
        self.time = datetime.datetime.now()
        self.strsecond = self.time.strftime("%S")
        self.second = int(self.strsecond)
        self.disappear()

        if self.snake.score >= 10 and self.cnt == 0:
            self.sleep = 0.1
            self.cnt +=1
    ...
    def disappear(self):
        if self.second % 6 == 0 and self.second !=0:
            self.apple.randompos()
            time.sleep(0.6)
